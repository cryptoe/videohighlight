			...
			$(".love-this", i).each(function(){
				$(this).click(function(){
					if(!$(this).hasClass("voted")) {									
						...
						var loveuri = ajax_url + "/vote.json";	
		...
		var clicked = function($this)
		{
			if(lockClick) {
				...
				if($this.find('.item-detail').length <= 0) {
					...
					var requri = ajax_url + "/" + $this.attr('data-id') + ".htm";					
					...
					var loadData = function() {
						$.ajax({
							...
							success: function(data) {			
								var response = {
									content : data,
									title : $(data).filter('.item-detail').data('title'), 
									type : $(data).filter('.item-detail').data('type'),
									love : $(data).filter('.item-detail').data('love'),
									voted : $(data).filter('.item-detail').data('voted')
								};
