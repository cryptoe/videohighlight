...
(function(window, $)
{
	...
	ImageSlider.eventTypes = [ImageSlider.INDEX_CHANGE, ImageSlider.ZOOM, ImageSlider.AUTOPLAY, ImageSlider.DRAG, ImageSlider.SLIDE_RESET];
	
	function ImageSlider(slider, _slides, varsObj)
	{
		TweenLite.defaultOverwrite = "auto";
		var _vars = $.extend({ minDragDistance:100, animDuration:0.3, maxZoom:5, minZoom:1, zoomMode:"screen", ease:Power4.easeOut, loop:true, allowZoom:true, allowDrag:true, doubleTapZoom:2, zoomStep:0.5, dragStep:10, adjustWidth:0, adjustHeight:0, playDuration:5, autoPlay:false, loadIndexOnly:true, preloaderUrl:"assets/preloader.gif", resizeDuration:-1, allowKeys:true }, varsObj),
			_slider = $(slider),
			...
			dragEvents = [],
			slideResetEvents = [],
			allEvents = [indexChangeEvents, zoomEvents, autoPlayEvents, dragEvents, slideResetEvents],
		...
		TweenMax.set(_slidesHolder, { transformPerspective:500 } );
				...
				if(len > 0)
				{
					for(var i = 0; i < len; i++)
					{
						...
						if(slide.url !== null)
						{
							...
							_slides[i].retry = 0;
							_slides[i].image = $("<img style='position:absolute; max-width:none;'>").load( { index:i }, onImageLoad).error( { index:i }, onImageError );
					...
					if(_vars.loop)
					{
						if(len > 0 && len <= 2)
						{
							...
							if(len == 1)
							{
								appendImage1 = $("<img style='position:absolute'>").load( { index:len }, onImageLoad).error( { index:len }, onImageError );
								url1 = _slides[0].url;
								caption1 = _slides[0].caption;
								
								appendImage2 = $("<img style='position:absolute'>").load( { index:len + 1 }, onImageLoad).error( { index:len + 1 }, onImageError );
								url2 = _slides[0].url;
		...
		function onPreloaderImageLoad(e)
		{
			...
			TweenMax.set([prevPreloader, curPreloader, nextPreloader], { position:"absolute", maxWidth:"none", x:preloaderX, y:preloaderY} );
		}
		...
		function onImageLoad(e)
		{
			...
			TweenMax.to(_slides[i].image, _vars.animDuration, { autoAlpha:1, ease:_vars.ease, transformPerspective:500 } );
				...
				if(len > 0)
				{
					for(var i = 0; i < len; i++)
					{
						...
						if(thumb.url !== null)
						{
							...
							_thumbs[i].image = $("<img style='position:absolute; max-width:none;'>").load( { index:i }, onImageLoad);
...
(function(window, $)
{
	...
	{
		var _vars = $.extend( { initShow:true, setHolderVisibility:true, animDuration:0.25, showCssObj:{ autoAlpha:1, delay:0.1 }, hideCssObj:{ autoAlpha:0, delay:0.1 } }, varsObj),
			isShow = _vars.initShow,
		...
		function _show(value, duration)
		{
			...
			if(value !== undefined)
			{
				...
				if(isShow)
				{
					if(setHolderVisibility)
					{
						tl.add(TweenMax.to(sliderDiv.parent(), 0, { display:"block", overwrite:"all" }));
					}
				...
				else
				{
					...
					if(setHolderVisibility)
					{
						tl.add(TweenMax.to(sliderDiv.parent(), 0, { display:"none", overwrite:"all", immediateRender:false }));
					}
				}
	...
	TouchNSwipe.init = function()
	{
		...
		for(i = 0; i < sliderHolderLen; i++)
		{
			...
			obj.touchNSwipe = new TouchNSwipe(sliderHolder, Utils.getAttrObjectFromString(sliderHolder.data("options")));
